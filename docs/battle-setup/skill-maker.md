...
**Note**: This option is not available when "None", "All", or "Mix" is selected.
...
**Note**: NP1 is always available, so setting minimum to 1 will match any servant.

---

## Grand Servant Support (JP Server Only)

Grand Servants are a special feature on the Japanese server where players can equip up to 3 Craft Essences on a single support servant.

### Grand Servant Options
...
**Warning**: Grand Servant detection requires the servant to be visible in the Grand Servant format on your support list.
...
### For Grand Servants (JP Server Only)
...
**Tip**: CE images should be created using the **Support Image Maker** feature for best results.
...
- Grand Servant is JP server only
